...
function Navbar() {
  ...
  return (
    <nav ref={navbarRef} className={`h-14 lg:h-16 flex justify-between item-center ${isDarkMode ? 'bg-gray-900' : 'bg-blue-50'} w-full px-2 sm:px-6 py-2 lg:py-0 lg:px-12`}>
      ...
      <Link to='/' className={`px-4 my-2 py-2 h-fit italic tracking-wide ${isDarkMode ?'bg-pink-400 text-slate-700':'bg-blue-500 text-white'} text-lg text-white rounded-2xl hidden lg:flex items-center justify-center hover:bg-blue-600 transition-all duration-300 transform hover:scale-105 active:scale-95 hover:transition-transform hover:duration-300 hover:transition-delay-150`}>
        Shopify
      </Link>
      {nav && (
        <div className='capitalize grid lg:hidden text-xs w-40 bg-blue-50 rounded-lg p-2 mt-2 ml-2 absolute top-10 left-0 sm:left-4 z-10'>
          <Menu />
        </div>
      )}
      ...
      <Link to='/Carts' className='p-3 active:scale-95 rounded-full  text-2xl transition transform duration-300 delay-100 relative'><IoCartOutline className={`${isDarkMode ?'text-gray-400':'text-gray-800'}`}/>
