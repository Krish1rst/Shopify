            ...
            {cart.map((item)=>(
                <div className='flex flex-col  bg-slate-50 p-2 md:flex-row w-full relative rounded-2xl' key={item.id}>
                    ...
                    <div className='flex flex-col items-center text-blue-500 min-w-fit mx-2 '>
                      <div className='flex md:flex-col gap-4 md:gap-0 md:items-center'>
                        <button className='text-blue-500 text-2xl px-2 py-1 rounded-md bg-transparent  hover:bg-blue-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 active:bg-blue-700 active:text-white'><MdKeyboardArrowUp /></button>
                        <p className='text-lg  text-gray-700 font-normal py-2'>{item.amount}</p>
                        <button className='text-blue-500 text-2xl px-2 py-1 rounded-md bg-transparent  hover:bg-blue-500 hover:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 active:bg-blue-700 active:text-white'><MdKeyboardArrowDown /></button>
                      </div>
                        <button className="text-blue-700 hover:underline text-sm font-normal text-left md:mt-10"onClick={()=>remove(item.id)}>remove
                        </button>
                    </div>
                    
                    <div className='w-full md:w-1/4 flex justify-center lg:justify-end text-lg md:text-sm text-gray-700 font-normal italic lg:pr-4'>Cost : ${item.price}
                    </div>
